...
Earlier AMD64 and ARM64 builds used the pinned Noetic base. A focused
network-isolated HTTP/ROS check passed the three profile requests, immediate
zero during a pause and stopping at the next red line. In that synthetic
three-second pause, the observed interval between first zero and resumed wheel
request was 3.057 seconds. A separate broader ROS run stopped at an existing
sharp-right assertion before reaching its chat section; that run is not
reported as a complete pass. The focused test establishes the changed chat
transport path independently.
...
and a straight override at the first red line. The latest attempt on
10 September 2026 appeared promising to the operator but ended because the
battery depleted. Its source snapshot is preserved and its result remains
\emph{incomplete}, not a successful end-to-end demonstration.
...
The final integrated scenario uses the A$\rightarrow$E straight, a seven-second
chat pause, a straight override at E, lane following toward C, and an operator
left instruction at C before stopping at the C$\rightarrow$B red line. Its
software flow is implemented; the battery-interrupted run does not complete
its physical validation.
...
The current scenario launcher selects the route and leaves the message box
empty. The operator types every pause and direction message. Camera-only mode
...
\bibitem{projectrepo}
T. Kezdekbayev, ``AlTaTa Duckietown Project,'' software repository, 2026.
\url{https://github.com/Tair1771/AlTaTa-Duckietown-Project}
\end{thebibliography}
